...
import { VideoCameraIcon, StopCircleIcon, MicrophoneIcon } from './common/Icons';
...
const MOCK_REALTIME_FEEDBACK = [
  ...
  { type: 'bodyLanguage', value: 'Good eye contact, but try to use more hand gestures.'},
  ...
  const handleStartInterview = async () => {
    ...
    if (stream) {
      ...
      timerRef.current = setInterval(() => {
        const randomFeedback = MOCK_REALTIME_FEEDBACK[Math.floor(Math.random() * MOCK_REALTIME_FEEDBACK.length)];
        setFeedbackLog(prev => [...prev, { ...randomFeedback, timestamp: Date.now() }]);
      }, 15000);
    }
  };
      ...
      case 'idle':
        return (
          <div className="text-center">
            ...
            <h2 className="text-xl font-bold text-gray-800 dark:text-white">AI Mock Interview</h2>
            <p className="mt-2 text-gray-600 dark:text-gray-300">Practice for your Product Manager interview. Our AI will provide feedback on your performance. You can say "Next question" or "Finish interview" to navigate.</p>
          ...
          <div>
            <div className="relative mb-4 bg-black rounded-lg overflow-hidden aspect-video">
                ...
                <div className="absolute top-2 left-2 flex items-center space-x-2">
                    <div className="bg-red-600 text-white px-2 py-0.5 text-xs font-bold rounded flex items-center">
                        <span className="h-2 w-2 bg-white rounded-full mr-1.5 animate-pulse"></span>REC
                    </div>
                     <div className={`p-1.5 rounded-full ${isListening ? 'bg-green-500/80' : 'bg-gray-500/80'}`}>
                         <MicrophoneIcon className="h-4 w-4 text-white" />
                     </div>
                </div>
                <p className="absolute bottom-2 left-2 text-white bg-black/50 px-2 py-1 rounded-md text-xs">Voice Commands: "Next question" | "Finish interview"</p>
            </div>
            <Card className="bg-gray-50 dark:bg-gray-900/50">
              <p className="text-sm text-gray-500 dark:text-gray-400">Question {currentQuestionIndex + 1} of {MOCK_INTERVIEW_QUESTIONS.length}</p>
              <p className="font-semibold text-lg text-gray-800 dark:text-white mt-1">{currentQuestion.text}</p>
            </Card>
            <div className="flex justify-between items-center mt-4">
              ...
              <Button 
                ...
                variant="light"
                disabled={currentQuestionIndex >= MOCK_INTERVIEW_QUESTIONS.length - 1}
              >
                  Next Question
              </Button>
            </div>
      ...
      case 'processing':
        return (
          <div className="text-center py-16">
            <Spinner />
            <p className="mt-4 font-semibold text-gray-700 dark:text-gray-200">Analyzing your performance...</p>
            <p className="text-sm text-gray-500 dark:text-gray-400">Our AI is generating your detailed report.</p>
          </div>
        );
      case 'finished':
        return report ? (
          ...
          <div className="text-center py-16">
            <p className="text-red-500">Could not generate your report. Please try again.</p>
