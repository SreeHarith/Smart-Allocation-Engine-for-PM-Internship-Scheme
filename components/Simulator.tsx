...
import { generatePersonalizedNotification } from '../services/geminiService';
...
const Simulator: React.FC<SimulatorProps> = ({ student, addNotification }) => {
  ...
  const handleStartSimulation = async () => {
    ...
    if (!student.skills.includes(newSkill)) {
        ...
        const relevantInternship = INTERNSHIPS.find(i => i.requiredSkills.includes(newSkill));
        ...
        if (relevantInternship) {
            ...
            const newScore = calculateMatchScore(studentWithNewSkill, relevantInternship);
            const improvement = newScore - initialScore;

            if (improvement > 0) {
                 scoreImprovementNotificationText = await generatePersonalizedNotification(student, {
    ...
    if (scoreImprovementNotificationText) {
        addNotification({ message: scoreImprovementNotificationText, type: 'success' });
    } else {
  ...
  return (
    <div className="p-4 bg-secondary-50 dark:bg-secondary-900/20 rounded-lg">
      <p className="text-gray-600 dark:text-gray-300 mb-4">
        Complete a mini-project to showcase your skills. Our AI will evaluate your performance, and the score will be considered in your final allocation.
      </p>
      <div className="border border-gray-200 dark:border-gray-700 p-4 rounded-md mb-4">
        <h4 className="font-semibold text-gray-800 dark:text-white">Task: Market Analysis Report</h4>
        <p className="text-sm text-gray-500 dark:text-gray-400">Analyze the provided dataset and create a one-page report on market trends for Q3.</p>
      </div>
      
      {score === null ? (
        <Button onClick={handleStartSimulation} disabled={isSimulating} className="w-full sm:w-auto">
        ...
        <div className="text-center bg-green-100 dark:bg-green-900/50 p-4 rounded-lg">
            <p className="font-bold text-green-800 dark:text-green-200">Evaluation Complete!</p>
            <p className="text-2xl font-bold text-green-600 dark:text-green-300">Your Score: {score}/100</p>
            <p className="text-sm text-green-700 dark:text-green-400">This score has been added to your profile.</p>
        </div>
      )}
    </div>
  );
