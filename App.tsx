...
import { ADMIN_DATA, COMPANY_DATA, STUDENT_DATA, INTERNSHIPS, NEW_INTERNSHIP_FOR_SIMULATION } from './constants';
...
const App: React.FC = () => {
  ...
  const addNotification = (notification: Omit<Notification, 'id' | 'userType' | 'read'>) => {
    const newNotification: Notification = {
      id: Date.now() + Math.random(),
      userType: currentUser?.role.toLowerCase() as 'student' | 'company' | 'admin' || 'student',
      read: false,
      ...notification,
    };
  ...
  useEffect(() => {
    if (currentUser?.role === 'STUDENT') {
      ...
      const showWelcomeNotification = async () => {
        const welcomeMessage = await generatePersonalizedNotification(student, { type: 'welcome' });
      ...
      const showDeadlineReminder = async () => {
        ...
        if (upcomingInternships.length > 0) {
          const bestUpcomingMatch = upcomingInternships.reduce((best, current) => {
            const bestScore = calculateMatchScore(student, best.internship);
            const currentScore = calculateMatchScore(student, current.internship);
            return currentScore > bestScore ? current : best;
          });
          
          const reminderMessage = await generatePersonalizedNotification(student, {
